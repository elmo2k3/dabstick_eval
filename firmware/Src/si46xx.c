...
/* #endif */
#define SI46XX_PIN_NSS GPIO_PIN_15
#define msleep(x) HAL_Delay(x)
#define SPI_Write(data,len) HAL_SPI_Transmit(&hspi1,data,len,1000)
#define SPI_Read(data,len) HAL_SPI_Receive(&hspi1,data,len,1000)
...
static void strip_trailing_ws(char *str)
{
	uint8_t i;
	uint8_t len;
...
static void si46xx_dab_parse_service_list(uint8_t *data, uint16_t len)
{
	...
	while(service_num < dab_service_list.num_services){
		...
		strip_trailing_ws(dab_service_list.services[service_num].service_label);
...
void si46xx_dab_start_digital_service_num(uint32_t num)
{
	...
	si46xx_dab_start_digital_service(dab_service_list.services[num].service_id,
			dab_service_list.services[num].component_id[0]);
	si46xx_status.dab_service_num = num;
}
...
void si46xx_dab_set_freq_list(uint8_t num, uint32_t *freq_list)
{
	...
	si46xx_status.dab_num_channels = num;
...
void si46xx_dab_tune_freq(uint8_t index, uint8_t antcap)
{
	...
	print_hex_str(buf,4);
	si46xx_status.dab_channel = index;
}
...
void si46xx_fm_rsq_status(struct fm_rsq_status_t *fm_status)
{
	uint8_t data = 0;
	char buf[20];
	...
	fm_status->valid = buf[5] & 0x01;
	fm_status->frequency = (buf[7]<<8 | buf[6])*10;
	fm_status->snr = (int8_t)buf[10];
	fm_status->rssi = (int8_t)buf[9];
	fm_status->freq_offset = (int8_t)buf[8]*2;
	fm_status->read_ant_cap = (int8_t)(buf[12]+(buf[13]<<8));
	/* printf("SNR: %d dB\r\n",(int8_t)buf[10]); */
	/* printf("RSSI: %d dBuV\r\n",(int8_t)buf[9]); */
...
void si46xx_dab_get_time(struct tm *thetime)
{
	uint8_t data[1];
	char buf[11];
	uint8_t timeout = 100;
	...
	timeout = 10;
	while(--timeout){
		data[0] = 0; // 0: local time 1:UTC
		si46xx_write_data(SI46XX_DAB_GET_TIME,data,1);
		si46xx_read(buf,11);
		if(buf[0] & 0x81){
			break;
		}
	}
	thetime->tm_year = (buf[4] | (buf[5]<<8)) - 1900;
	thetime->tm_mon = buf[6]-1;
	thetime->tm_mday = buf[7];
	thetime->tm_hour = buf[8];
	thetime->tm_min = buf[9];
	thetime->tm_sec = buf[10];
	mktime(thetime);
}
...
void si46xx_dab_scan()
{
	...
	for(i=0;i<si46xx_status.dab_num_channels;i++){
...
static void si46xx_set_standard_parameters(void)
{
	...
	si46xx_set_property(SI46XX_DIGITAL_IO_OUTPUT_SAMPLE_RATE,46877);
}

void radio_dab(){
	si46xx_init_dab();
	/* msleep(1000); */
	si46xx_set_standard_parameters();
	...
	si46xx_dab_tune_freq(1,0);
	/* si46xx_dab_start_digital_service(0x0000d392,0x00000007); // 92 D3 00 00    07 00 */
}
